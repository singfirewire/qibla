...
const QiblaFinder = () => {
  ...
  const compassRotation = compass ? `rotate-[${compass}deg]` : '';
  const qiblaRotation = qiblaDirection ? `rotate-[${qiblaDirection}deg]` : '';
        ...
        {error ? (
          ...
          <div className="space-y-4">
            ...
            <div className="relative w-48 h-48 mx-auto">
              {/* เข็มทิศพื้นหลัง */}
              <div className={`absolute inset-0 transition-transform duration-200 ${compassRotation}`}>
                <Compass size={192} className="text-gray-400" />
              </div>
              
              {/* ลูกศรชี้ทิศกิบลัต */}
              {qiblaDirection && (
                <div className={`absolute inset-0 transition-transform duration-200 ${qiblaRotation}`}>
